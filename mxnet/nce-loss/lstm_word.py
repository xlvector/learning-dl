...
class NceAuc(mx.metric.EvalMetric):
    ...
    def update(self, labels, preds):
        preds = np.array([x.asnumpy() for x in preds]).transpose()
        label_weight = labels[1].asnumpy().reshape((preds.shape[0], preds.shape[1]))
        tmp = []
        for i in range(preds.shape[0]):
...
if __name__ == '__main__':
    ...
    data_train = DataIter("./data/text8", batch_size, num_label, num_label,
                          init_states)
    
    network = get_net(data_train.vocab_size, num_label, num_label, num_lstm_layer, num_hidden)
